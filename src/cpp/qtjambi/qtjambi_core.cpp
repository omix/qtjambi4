...
bool qtjambi_initialize_vm(const QStringList & vm_parameters, const bool ignoreUnrecognizedOptions)
{
    ...
    if (libvm.isEmpty()) {
// Framework JavaVM no longer available in OS X >= 10.10
//#ifdef Q_OS_DARWIN
...
#if defined( Q_WS_WIN )
			// in windows, lvm.dll will not load without previously loading
			// msvcr100.dll from the upper directory (%JAVA_HOME%/bin).
			QDir libdir = QFileInfo(libpath).dir();
			libdir.cdUp();
			QFileInfoList msvcrList = libdir.entryInfoList(QStringList() << "msvcr*.dll");
			foreach(QFileInfo msvcrFile , msvcrList){
				QLibrary msvcrLib(msvcrFile.absoluteFilePath());
				msvcrLib.load();
			}
#endif
