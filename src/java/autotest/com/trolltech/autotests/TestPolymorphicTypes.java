...
public class TestPolymorphicTypes extends QWidget {
	...
	public void testPolymorphicQGraphicsItem() {
		...
		try {
			QGraphicsItemInterface item9 = (QGraphicsItemInterface)Class.forName("com.trolltech.qt.declarative.QDeclarativeItem").newInstance();
			scene.addItem(item9);
		} catch (Throwable e) {}
		try {
			QGraphicsItemInterface item10 = (QGraphicsItemInterface)Class.forName("com.trolltech.qt.qt3dquick.QDeclarativeItem3D").newInstance();
			scene.addItem(item10);
		} catch (Throwable e) {}
		try {
			QGraphicsItemInterface item11 = (QGraphicsItemInterface)Class.forName("com.trolltech.qt.svg.QGraphicsSvgItem").newInstance();
		...
		for (QGraphicsItemInterface item : scene.items()) {
			if(item.toGraphicsObject()!=null){
				...
				assertFalse("class is not ConcreteWrapper "+item.getClass().getName(), item.getClass().getName().equals("com.trolltech.qt.gui.QGraphicsItem$ConcreteWrapper"));
			}
		}
    }
	...
	public void testPolymorphicQLayoutItem() {
		...
		for (int j = 0; j < layout.count(); j++) {
			...
			assertFalse("class is not ConcreteWrapper "+item.getClass().getName(), item.getClass().getName().equals("com.trolltech.qt.gui.QLayoutItem$ConcreteWrapper"));
		}
    }
