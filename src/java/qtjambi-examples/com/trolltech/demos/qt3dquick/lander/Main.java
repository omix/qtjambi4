...
**
****************************************************************************/
package com.trolltech.demos.qt3dquick.lander;
...
@QtJambiExample(name = "Qt3DQuick Lander", canInstantiate = "call-static-method:checkQt3DQuickSupport")
public class Main extends QMainWindow{
	...
	private static QWidget createView(QMainWindow parent){
		...
		if(parent!=null){
		    ...
		    view.setWindowTitle("Lander");
		    view.setWindowIcon(new QIcon("classpath:/com/trolltech/demos/qt3dquick/quick3d.png"));
		}
	    view.setSource(new QUrl("classpath:/com/trolltech/demos/qt3dquick/lander/qml/lander.qml"));
	...
	/**
	 * Since the AiLoader plugin cannot load meshes from network
	 * we must load each *.3ds mesh manually from QIODevice here. 
	 * @param view
	 */
	private static void loadMeshes(QDeclarativeView3D view) {
		loadMesh(view, "lunar-lander.3ds", "landerMesh", "lunar_lander", "3ds");
	}
	
	private static void loadMesh(QDeclarativeView3D view, String filename, String objectName, String varname, String format) {
		QFile file = new QFile("classpath:/com/trolltech/demos/qt3dquick/lander/qml/meshes/"+filename);
		file.open(QIODevice.OpenModeFlag.ReadOnly);
		QGLAbstractScene scene = QGLAbstractScene.loadScene(file, new QUrl("file:"+filename), format);
		file.close();
		if(scene!=null){
			QDeclarativeMesh mesh = new QDeclarativeMesh(view);
			mesh.setObjectName(objectName);
			mesh.setScene(scene);
