...
void JavaGenerator::writeEnum(QTextStream &s, const AbstractMetaEnum *java_enum) {
    ...
    if (entry->isExtensible()) {
        ...
        << INDENT << "            e = (" << java_enum->name() << ") com.trolltech.qt.GeneratorUtilities.createExtendedEnum("
        ...
        s << INDENT << "        throw new com.trolltech.qt.QNoSuchEnumValueException(value);" << endl;
    }
    ...
    if (spec && java_function->modifiedName() == java_function->originalName()) {
        if (java_function->isPropertyReader()) {
            ...
            if (!spec->stored().isEmpty())
                s << "    @com.trolltech.qt.QtPropertyStored(\"" << spec->stored() << "\")" << endl;
//   This seams to be unnecessary in QtJambi
//            if (!spec->revision().isEmpty())
//                s << "    @com.trolltech.qt.QtPropertyRevision(\"" << spec->revision() << "\")" << endl;
            if (!spec->user().isEmpty())
                s << "    @com.trolltech.qt.QtPropertyUser(\"" << spec->user() << "\")" << endl;
...
void JavaGenerator::write(QTextStream &s, const AbstractMetaClass *java_class, int nesting_level) {
    ...
    if(nesting_level==0){
       if (m_docs_enabled) {
        ...
        s << INDENT << "import com.trolltech.qt.*;" << endl << endl;
   ...
   if (m_doc_parser) {
...

    {
        ...
        if (!java_class->isFinalInTargetLang() && java_class->isFinalInCpp()) {
            ...
            << INDENT << " **/" << endl
            << INDENT << "protected " << java_class->simpleName() << "() throws QClassCannotBeSubclassedException {" << endl
...
void JavaGenerator::writeFunctionAttributes(QTextStream &s, const AbstractMetaFunction *java_function,
    ...
    if ((options & SkipAttributes) == 0) {
        ...
        if (spec && java_function->isPropertyNotify()) {
            s << "    @com.trolltech.qt.QtPropertyNotify(name=\"" << spec->name() << "\")" << endl;
        }
